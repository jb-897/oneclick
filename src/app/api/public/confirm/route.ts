  ...
  try {
    const [row] = await db
      ...
      .where(
        and(
          ...
          gt(registrations.confirmationTokenExpiresAt, sql`now()`)
        )
      )
      .limit(1);
    ...
    if (rows.length > 0) {
      return NextResponse.redirect(
        new URL(`/confirm?success=1&sessionId=${encodeURIComponent(row.sessionId)}`, req.url)
      );
    }

    return NextResponse.redirect(
      new URL(`/confirm?error=full&sessionId=${encodeURIComponent(row.sessionId)}`, req.url)
    );
  } catch (e) {
